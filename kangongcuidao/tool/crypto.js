 ...
 * @retrun string 加密后的字符串
 * */
exports.getEncAse128 = function(str, secret) {
	let cipher = crypto.createCipher("aes-128-ecb", secret); //设置加密类型 和 要使用的加密密钥
	let enc = cipher.update(str, "utf8", "hex");    //编码方式从utf-8转为hex;
	enc += cipher.final("hex"); //编码方式从转为hex;
	return enc; //返回加密后的字符串
};
 ...
 * @retrun string 解密后的字符串
 * */
exports.getDecAse128 = function(str, secret) {
	if(str){
		let decipher = crypto.createDecipher("aes-128-ecb", secret);
		let dec = decipher.update(str, "hex", "utf8");//编码方式从hex转为utf-8;
		dec += decipher.final("utf8");//编码方式从utf-8;
		return dec;
	}else{
